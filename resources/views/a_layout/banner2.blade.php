...
<div class="row">
	<div class="col-md-12">
		<div class="widget">
			<form role="form" id="OtherValidator" method="POST" action="{{route('cbanner2_post')}}" enctype="multipart/form-data">
			{{csrf_field()}}
			<table height="250px" width="50%" >
				<tr>
					<td>
						SELECT IMAGE :
					</td>
			...
			<div class="widget-header">
				<h2><strong>Edit </strong>Second Slider</h2>
			...
			<div class="widget-content">
				...
				<div class="table-responsive">
					<form class='form-horizontal' role='form'method="post" action="{{route('cbanner2_post')}}" enctype="multipart/form-data">
						{{csrf_field()}}
						<table id="datatables-4" class="table table-striped table-bordered" cellspacing="0" width="100%">
							...
							<tbody>
								...
								<tr>
									<td>{{$d->id}}</td>
									<td>{{$d->name}}</td>
									<td><img src="{{url('storage/app/banner2')}}{{'/'}}{{$d->image}}" style="height: 60px; width: 60px;"></td>
									<td>{{$d->price}}</td>
									<td>
										@if($d->active == 'active')
											<a href="{{route('cbanner2_active',['id'=>encrypt($d->id),'active' => 'active'])}}">Active</a>
										@else
											<a href="{{route('cbanner2_active',['id'=>encrypt($d->id),'active' => 'inactive'])}}">Inactive</a>
										@endif
									</td>
									<td>{{$d->created_at}}</td>
									<td><center><a href="{{route('d_banner2',['id'=>$d->id])}}"><i class="fa fa-trash" aria-hidden="true"></i></a></center></td>
								</tr>
								@empty
								@endforelse
								@endif
